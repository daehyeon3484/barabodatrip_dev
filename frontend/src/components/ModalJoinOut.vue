<template>
  <div :class="isMobile == true ? 'mobile-lg' : ''"> 
    <button class="float-end btn btn-outline-warning me-md-2 text-light" hidden type="button" id="outModalBtn" data-bs-toggle="modal" data-bs-target="#staticBackdrop_out">test</button> 
    <!-- Modal -->
    <div class="modal fade modal-md" id="staticBackdrop_out" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel_out" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <div :class="isMobile == true ? 'mobile fw-bold text-start' : 'fw-bold text-start'" id="staticBackdropLabel_out">{{$t('common.join_out')}}</div>
            <button id="modalOutClose" type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <div class="fw-bold font-lg text-info text-center"> {{$t("common.join_out")}}</div>
            <div class="mb-2 row pt-2"> 
              <!-- <div class="input-group mb-2"> 
                <span class="input-group-text span-font col-auto" id="inputGroup-sizing-sm">{{ $t("order.name") }}</span>
                <input type="text" class="col-md-6 form-control mobile-md" ref="id_mem_nm" v-model.trim="idInfo.mem_nm" :placeholder="$t(`common.order_name_input`)" :aria-label="$t(`common.order_name_input`)">
              </div> -->
            <label  class="col-md-4 col-form-label  fw-bold text-start font-lg">* {{$t("order.name")}}</label>
              <div class="col-md-8">
            ...
            <div class="fw-bold font-lg text-info text-center"> {{$t("common.common_login_password_find")}}</div>
            <div class="mb-2 row pt-2">
              <div class="input-group mb-2"> 
          ...
          <div class="modal-footer">
            <button  type="button" class="btn btn-secondary mobile-ss btn-st-sm" @click="reset">{{ $t("common.common_close") }}</button>
          </div>
        </div>
      </div>
    </div>
